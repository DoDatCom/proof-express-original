...
*/
	...
	$pgQuery=mysqli_query($con,"SELECT id FROM pages");
	...
	send_message($serverTime, $pTotal.' of '.$pgTotal.' pages require update.','-','-'); // Archive
	...
	while($pgid=mysqli_fetch_assoc($pQuery)){
		...
		for($i=1;$i<20;$i++){
			...
			if($pgid["rev"]==$i){
				...
				send_message($serverTime, $pgInt.' -  UPDATED '.$pgid["name"].', version '.$i,$pgPercent,$etaText);
				...
				while($new=mysqli_fetch_assoc($newID)){
					...
					$secs=$etaEnd % 60;
					$etaText='ETA: '.intval($etaEnd/60).' minutes, '.$secs.' seconds';
					send_message($serverTime, $pgInt.' -  ADDED '.$pgid["name"].', version '.$i,$pgPercent,$etaText);
				}
			} else {
				break;
			}
