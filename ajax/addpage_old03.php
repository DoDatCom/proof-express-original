...
//			$ret = $dimScale;
			$gs=exec_timeout(GS_PATH.'gs -dBATCH -dNOPAUSE -r72 -sDEVICE=jpeg -dTextAlphaBits=4 -dGraphicsAlphaBits=4 -dJPEGQ=80 -g'.$dimScale.' -dPDFFitPage -sOutputFile="'.$dir.'IMG/'.$pg["id"].'.jpg" "'.$dir.'PDF/'.$pg["id"].'.pdf" 2>&1',30);
			if($gs!=='GS_OK'){
				exec('cp '.ABSDIR.'/images/logos/pdf_logo.jpg '.$dir.'IMG/'.$pg["id"].'.jpg');
			}
			$hrType="pdf";
			$hrImg=$dir.'PDF/'.$pg["id"].'.pdf';
			$imgImg=$dir.'IMG/'.$pg["id"].'.jpg';
		} else {
...
//	Retrieve list of users to be notified.
	...
	while($m=mysqli_fetch_assoc($uQuery)){
		if($m["email"]!==null && $m["email"]!=='' && $m["notify"]==1){
			...
			$mail->IsHTML(true);
			$mail->SMTPAuth = true;
//			$mail->Username = 'proofexpress@spartannash.com';
			$mail->Username = MAILUSER;
			$mail->Password = 'Support1026';
//			$mail->Host = 'spmta1.spartanstore.com';
