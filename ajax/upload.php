...
//	Process GET data.
	...
	$newEmailQ=mysqli_query($con,"SELECT * FROM ads WHERE id = ".$ad);
	while($send=mysqli_fetch_assoc($newEmailQ)){
		if($send["status"]=="New"){
			$statQ=mysqli_query($con,"UPDATE ads SET status = 'Active' WHERE id = ".$ad);
			$sendOnce=true;
		}
	}
	...
	if($sendOnce==true){
		...
		while($m=mysqli_fetch_assoc($uQuery)){
			if($m["email"]!==null && $m["email"]!=='' && $m["notify"]==1){
                ...
                $tQuery=mysqli_query($con,"UPDATE users SET status = CONCAT(IFNULL(status,''), '".$token.":') WHERE id = ".$m["id"]);
			$link=$url.'update.php?t='.dechex($m["id"]).':'.$token.':'.dechex($pid).":".dechex($fid).":".dechex($ad).":".dechex($pgid);
			...
			try {
				...
				$mail->Username = 'support@dodatcommunications.com';		// SMTP username
				$mail->Password = 'Support1026';									// SMTP password
				$mail->SMTPSecure = 'tls';												// Enable TLS encryption, `ssl` also accepted
				$mail->Port = 587;														// TCP port to connect to
					
				//Recipients
				$mail->setFrom('support@dodatcommunications.com', 'Proof Express');
				$mail->addAddress($m["email"], $m["fullname"]);					// Add a recipient
				...
				$mail->Subject = 'New ad from Proof Express';
				$mail->Body = '
<html>
	<head>
		<meta charset="utf-8">
	...
	<body>
        ...
        $close=mysqli_query($con,"UPDATE ads SET status = 'Active' WHERE id = ".$ad);
	}
